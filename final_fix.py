# -*- coding: utf-8 -*-
"""阶段4 帧清洁（mochi 柴犬 v118 验证版，幂等可重跑，零几何变化）。
...
pass 结构:
  ...
  6) bath 泡沫专用白化（fix_bath_shadows）
  7) Lab 色调对齐 idle 基准（fix_tone，CALIB_STATES 内状态）
  8) 青底残留清除（fix_teal，rembg 近景大脸帧偶留青块）
  9) 时序白色稳定（temporal_white_stable，PROP_INTERACT_STATES 首帧锚定态专用）
...
# ── 换宠物 CONFIG（三值，其余全自动）──
# 道具交互态（首帧锚定生成的状态，如 type 敲键盘）：清洁管线对其放宽——
# 键帽间隙/阴影洞/底座小CC是道具自然结构，逐帧被当洞填充/光环删除=道具白化+闪烁（mochi v101 根因）
PROP_SAFE_STATES = {'type'}
# 生成视频色偏需向 idle 基准对齐的状态（逐状态自动测 Lab 差平移校正，幂等；无偏状态勿加）
CALIB_STATES = {'roll', 'kiss', 'type', 'walk', 'run'}
# 首帧锚定态的时序白闪稳定（视频模型逐帧生成的白色镜面高光时隐时现）；与 PROP_SAFE_STATES 同源
# 所有状态都跑时序白色稳定（萨摩耶v128验证：白毛+白背心+白背景=全状态闪烁）
TEMPORAL_WHITE_STATES = {'idle', 'sit', 'eat', 'bark', 'happy', 'roll', 'dance', 'beg', 'bath',
                         'lick', 'surprised', 'play_dead', 'sleep', 'stretch', 'walk', 'run',
                         'pet', 'kiss', 'wave', 'type'}
# rembg 青底残留清除目标态（近景大脸帧偶留青块；mochi 实测吻部特写态中招）
TEAL_STATES = {'kiss'}

def fix_frame(a, prop_safe=False):
    """prop_safe=True: 道具交互态——跳过pass1洞填充/pass2 halo删除/pass5悬浮碎片，
    保留道具自然结构（键帽间隙/底座）。"""
    ...
    out = a.copy()
    core = al > 127
    # v86: 外推源排除低饱和灰(matting灰带)——否则semi灰环ext取灰带自身=硬化灰块
    ...
    # ── 1) core 封闭洞填充 ──
    # prop_safe 跳过——键帽间隙/阴影洞是道具自然结构,外推填充=道具白化
    if not prop_safe:
    ...
    if semi.any():
        ...
        out[halo, 3] = 0
        if not prop_safe:
            # 道具态半透边=道具AA渐变, ext外推主体色=道具边缘变色; 保留原RGB
            out[keep, :3] = ext[keep]

    # ── 3) rim 黑线删除: 轮廓外不透明暗像素（硬黑线非自然AA）──
    # prop_safe 用 closing 封闭 body——道具间隙 semi 致 body 漏孔, 深色道具被判 rim 删除=黑斑
    if prop_safe:
        body = binary_fill_holes(binary_closing(core, iterations=3))
    rim = (al > 0) & (lum < 50) & ~body
    out[rim, 3] = 0

    # ── 4) fringe 清零（prop_safe 跳过——道具阴影AA半透边是自然边缘）──
...
def fix_bath_shadows(a):
    ...
    E 泡沫内透明洞→补泡沫白：matting把白泡沫当白背景抠掉=桌面透出=时隐时现，
...
# ── pass7: Lab 色调对齐 idle 基准 ──
# 根因: 个别状态生成视频色偏(偏暗/发青/去饱和)。逐状态自动测主体Lab均值与idle基准差→平移校正。
# 纯像素色值变换零几何风险；幂等(重跑shift≈0)。高光保护防过曝。
def srgb2lab(rgb):
    rgb = rgb / 255.0
...
def ref_lab():
    """idle 基准=全部 idle 帧 Lab 均值（自动，无硬编码文件名）。"""
    global _REF_LAB
    if _REF_LAB is None:
        fs = sorted(glob.glob(f'{ASSETS}/idle_*.webp') + glob.glob(f'{ASSETS}/idle_*.png'))
        if not fs:
            return np.zeros(3)
        _REF_LAB = np.mean([_lab_mean(f) for f in fs], 0)
    return _REF_LAB
...
def fix_teal(a):
    """pass8: 青底残留清除。rembg对近景大脸帧偶留青块。
    青CC判据 G-R>12 & B-R>8 & R<190；连透明/帧边=背景残→alpha=0；
    主体内封闭洞(背景透出)→最近实体毛色外推+alpha=255(填洞保主体完整)。"""
    rgb = a[:, :, :3].astype(np.int32)
    r, g, b = rgb[..., 0], rgb[..., 1], rgb[..., 2]
    teal = (a[:, :, 3] > 0) & ((g - r) > 12) & ((b - r) > 8) & (r < 190)
    if teal.sum() < 20:
        return a
    ...
    _grayish = ((_mx - _mn) <= 45) & (_mn <= 195) & (_mx <= 235) & ((_mx - _mn) <= 0.22 * _mx)
    src = solid & ~_grayish & ~teal        # 排除青色自身(青alpha=255会自填)
...
def state_shift(st):
    fs = sorted(glob.glob(f'{ASSETS}/{st}_*.webp') + glob.glob(f'{ASSETS}/{st}_*.png'))
    if not fs:
        return np.zeros(3)
...
def temporal_white_stable(state):
    ...
    (非白基色); 稳定白(出现率>0.95, 如常驻眼高光)保留=自然。"""
...
def main():
    ...
    tot = 0
    # 青底残块预清(写盘)→后续 shift 测量不被青像素污染
    for st in TEAL_STATES & set(states):
        for fp in sorted(glob.glob(f'{ASSETS}/{st}_*.png') + glob.glob(f'{ASSETS}/{st}_*.webp')):
    ...
    for st in states:
        ...
        for fp in fs:
            im = Image.open(fp).convert('RGBA')
            a = np.asarray(im)
            o = fix_frame(a, prop_safe=(st in PROP_SAFE_STATES))
            if st == 'bath':
                o = fix_bath_shadows(o)
            if st in TEAL_STATES:
                o = fix_teal(o)                          # 青底残块清除(校色前)
            if st in CALIB_STATES:                       # 色调对齐idle基准
                o = fix_tone(o, shift)
            if not np.array_equal(a, o):
                save_retry(Image.fromarray(o, 'RGBA'), fp)
                tot += 1
        if st in TEMPORAL_WHITE_STATES:                  # 逐帧pass后时序白色稳定
