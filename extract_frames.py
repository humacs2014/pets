...
CANVAS = 1088                  # golden_vest_pet uses 1088x832 canvas (original video resolution)
GROUND = 818                   # walk/run 脚底线 (canvas 832 内, bottom aligned with GROUND_PAD=14)
...
TARGET_H = {
    # v96: all values scaled from 1024→1088 canvas (×1.0625)
    'idle': 580, 'bark': 580, 'happy': 578, 'dance': 578,
    'beg': 574, 'bath': 576, 'sit': 669,
    'stretch': 616,
    'walk': 540, 'run': 576,          # 步态侧身档
    'lick': 576, 'surprised': 578,
    'eat': 504,
    'pet': 580,
    'roll': 380, 'play_dead': 366,    # 躺卧档
}
# 重采样到引擎 ANIMS 声明帧数（引擎按 count 加载，帧数必须 1:1）
RT_FRAMES = {
    'idle': 101, 'eat': 34, 'bark': 57, 'sit': 63, 'roll': 121,
    'dance': 57, 'beg': 56, 'bath': 57, 'stretch': 117,
    'surprised': 45, 'play_dead': 68, 'sleep': 51, 'lick': 121,
...
def cutout_frames(fps, state):
    """v96-fix3: 用ben2+rembg双模型union抠图（与其他所有动作一致）。
    旧isnet单模型在身体内部低对比区(背心带子交叉处)产生透明洞。
    union方案: rembg/u2net前景mask主控(>128=前景)→无内部洞,
    BEN2在前景区补充精细alpha, RGB来自rembg(不经过alpha预乘=不模糊)。
    直接调用union_cutout_final.py的run_ben2+run_rembg+make_union。"""
    ...
    # Import union cutout functions
    from union_cutout_final import run_rembg, run_ben2, make_union, BG as U_BG, PAD as U_PAD
    import torch
    ...
    # Step 1: BEN2
    print(f'  [BEN2] cutting {len(fps)} frames...', flush=True)
    run_ben2(fps, work_dir)

    # Step 2: rembg (u2net)
    print(f'  [rembg u2net] cutting {len(fps)} frames...', flush=True)
    run_rembg(fps, work_dir)

    # Step 3: Union (v6: 需要raw帧RGB做白色判断)
    print(f'  [union v6] merging...', flush=True)
    ...
    for i in range(len(fps)):
        ...
        # v6: 加载raw帧RGB用于白色判断前景
        raw_rgb = np.array(Image.open(fps[i]).convert('RGB'))
        union_arr = make_union(rembg_arr, ben2_mask, raw_rgb=raw_rgb)
        Image.fromarray(union_arr, 'RGBA').save(outp)
        mats.append(outp)
        if i % 20 == 0:
            print(f'    union {i}/{len(fps)}', flush=True)

    # Cleanup work dir
    import shutil as shutil2
    if os.path.exists(work_dir):
        shutil2.rmtree(work_dir)
...
def harden_alpha(img):
    """v96-fix: gentler hardening. Old threshold v>100→255 was too aggressive,
    killing semi-transparent fur edge pixels (50-100 range) → jagged edges + detail loss.
    New: v>30→255 (preserve all fur, only kill near-transparent background residual).
    Pixels 0-30 = background noise/ghost → zero. Pixels 31-254 = real fur edges → full opaque."""
    a = img.getchannel('A')
...
def cleanup_debris(mats):
    """v3-union后碎片清理：BEN2在背景区输出零散高置信(alpha>150)像素，
    导致main_frac<0.99和edge=True，clean_window全拒。
    修复：只保留alpha>128的最大连通分量，删除其他碎片（覆写m_*.png）。"""
    ...
    for p in mats:
        ...
        debris = fg & (lab != main_label)
        if debris.sum() > 0:
            arr[debris, 3] = 0  # zero out debris alpha
            Image.fromarray(arr, 'RGBA').save(p)
            cleaned_count += 1
    if cleaned_count > 0:
        print(f'  cleanup_debris: cleaned {cleaned_count}/{len(mats)} frames', flush=True)
    return mats
