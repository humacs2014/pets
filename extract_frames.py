...
TARGET_H = {
    ...
    'stretch': 580,
    'walk': 560, 'run': 542,          # walk对齐idle档(用户报walk明显小于其他状态); run保持
    'lick': 542, 'surprised': 544,
    'eat': 474,
    'pet': 546,   # 摸摸头: 站姿档(视频为四腿站立3/4视), 与idle/bark同档防忽大忽小
    'roll': 630, 'play_dead': 344,    # roll对齐主体档(用户报roll比主体小)
    'kiss': 630, 'wave': 630, 'type': 640,  # kiss/wave坐姿档=sit 630; type含键盘整体bbox 640
}
# 重采样到引擎 ANIMS 声明帧数（引擎按 count 加载，帧数必须 1:1）
RT_FRAMES = {
    'idle': 121, 'eat': 34, 'bark': 57, 'sit': 63, 'roll': 121,  # v22g: idle/roll=121匹配引擎ANIMS（旧101/47不匹配→手动rembg绕行=质量灾难）
    'dance': 57, 'beg': 56, 'bath': 57, 'stretch': 117,
    'surprised': 45, 'play_dead': 68, 'sleep': 51, 'lick': 54,
    'happy': 121, 'pet': 107,  # v23: walk移出RT_FRAMES——走run路线用长循环原生帧(35帧@42ms≈1.5s循环=自然步态)，resample到121=3.5倍慢放=滑行
    # v100: run移出RT_FRAMES——36原生帧resample 15=2.4:1抽帧+1T=0.81s短循环=用户报"卡/重复感"，改长循环原生帧
    'kiss': 121, 'wave': 57, 'type': 86,  # type走find_loop_pair动态决定帧数(跳过resample)，此值仅注释用
...
def harden_alpha(img, threshold=100):
    a = img.getchannel('A')
...
def process_state(name):
    ...
    # drop_whitebg only for states that have non-white intro frames to skip
    # roll/type on white bg → all frames have white corners → skip drop_whitebg
    fps = fps  # no-op placeholder
    ...
    if name in ('sleep', 'stretch'):
        ...
        if name in ('walk', 'run'):
            ...
            if name in ('type', 'kiss', 'roll'):
                ...
                # (丢走近/舔镜头), 同type全窗。
                # v103 roll: 一次性动作(站→躺→翻→站), clean_window丢弃前22帧站立段=动作
                # 不完整。全窗保留完整动作。
                s, e = 0, len(mats)
    ...
    if name in PINGPONG:
        # v22g: 如果frames*2-1 >= target_len，截取正播段到正好拼出target_len帧
        # 避免resample引入帧间跳变闪烁（idle从72帧ping-pong→143帧resample121=闪烁）
        target_len = RT_FRAMES.get(name, 0)
        n_frames = len(frames)
        if target_len > 0 and n_frames * 2 - 1 >= target_len:
            # ping-pong: fwd[0..half-1] + rev[half-2..0] = half + (half-1) = 2*half-1
            # 2*half-1 = target_len → half = (target_len+1)/2
            half = (target_len + 1) // 2
            fwd = frames[:half]
            rev = frames[half - 2::-1]  # 倒播从half-2到0（极值帧half-1只出现一次）
            seq = fwd + rev
            print(f'  ping-pong exact-fit: {half} fwd + {len(rev)} rev = {len(seq)} frames (no resample)', flush=True)
        else:
    ...
    if name in ('walk', 'run'):
        ...
        if name in PINGPONG or name in ONESHOT:
            # v-samoyed 铁律: ping-pong/oneshot seq 上面已组装(正播+倒播/全帧直放),
            # 禁被本 else 分支的 find_loop_pair 覆盖——萨摩耶 idle 微幅视频
            # loop pair 仅 11 帧 resample 101 = 9倍重复帧 = 用户可见卡顿。
            # 同类死代码: v88 修了 roll, v100 修了 kiss, 但 PINGPONG/ONESHOT
            # 全集仍被覆盖直到本次。旧宠物未暴露=loop pair 碰巧足够长。
            print(f'  keep {name} seq as assembled ({len(seq)} frames)', flush=True)
    ...
    for k, f in enumerate(seq):
        # v22g: idle/walk用低阈值harden(30)减少边缘闪烁——alpha在30-100间抖动的像素
        # 被强制二值化导致帧间出现/消失跳变。低阈值让更多边缘像素保留。
        # 其他状态保持默认100。
        threshold = 30 if name in ('idle', 'walk') else 100
        im = harden_alpha(f, threshold=threshold)
        if name == 'bath':
            im = harden_foam(im)
