# -*- coding: utf-8 -*-
"""阶段6 定量审计门（与 final_fix 清洁定义一致，clean=pass）。
真缺陷定义:
...
用法: env -u PYTHONPATH -u PYTHONHOME <anaconda>/python.exe audit_quant.py
"""
import os, glob
import numpy as np
...
# v110c samoyed: 同步自 samoyed_pet.py ANIMS（全窗121/逐态实测）
ANIMS = {'idle': 48, 'walk': 44, 'run': 80, 'eat': 121, 'bark': 121,
...
STATIC = {'idle', 'sit', 'beg'}                             # 相邻突变<6%
# v110c samoyed: lick=0-70站→坐过渡+71-120舔毛loop，姿态大过渡豁免
TRANS = {'sleep', 'roll', 'stretch', 'play_dead', 'eat', 'lick'}
...
for st, want in ANIMS.items():
    ...
    if not fs:  # v63: webp资产
        fs = sorted(glob.glob(os.path.join(ASSETS, st + '_*.webp')))
    if len(fs) != want:
    ...
    elif st in JUMP_EXEMPT:
        jtag, jlim = 'rhythm-exempt', 999
    elif st in STATIC:
        jtag, jlim = 'static', 6
    else:
