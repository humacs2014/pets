#!/usr/bin/env python3
"""
拉布拉多桌面宠物 v6.0 — 混合渲染引擎（竞品同款技术路线）
=========================================================
核心原理（与竞品Deskpet Dog完全一致）：
  1. 原版拉布拉多精灵图作为基底（保留原版神韵）
...
#  主窗口
# ═══════════════════════════════════════════════════════════
WINDOW_TITLE = 'LabradorDesktopPet_MainWindow'  # 固定窗口标题：供重复启动的新实例FindWindow定位并激活
...
def _acquire_single_instance():
    """Windows命名互斥锁：防止用户双击多次导致多只拉布拉多/僵尸进程"""
    if sys.platform != 'win32':
        return True
    import ctypes
    kernel32 = ctypes.windll.kernel32
    global _singleton_mutex
    _singleton_mutex = kernel32.CreateMutexW(None, False, 'LabradorDesktopPetV7_Singleton')
...
def _find_existing_window():
    ...
    （旧版本EXE的窗口标题是PyInstaller按EXE名设置的"拉布拉多桌面宠物vN"）"""
    import ctypes
    ...
    def _cb(hwnd):
        ...
        if n:
            ...
            if buf.value.startswith('拉布拉多桌面宠物'):
                found.append(hwnd)
        return True
